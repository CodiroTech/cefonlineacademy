...
const DonateCard1: React.FC<DonateCardProps> = ({
  ...
  const isSelectable = selectedPayment && selectedDonationType && (amount || inputAmount)

  const { addToCart } = useDonationCart()
  const { showToast } = useToast()

  const handleAddToCart = () => {
    const donationAmount = amount ? amount : inputAmount.trim()
    ...
    const item: DonationCartItem = {
      id: toNumericId(`${donationAmount}-${selectedPayment}-${selectedDonationType}`),
      title: 'Wherever Most Needed',
        ...
        <div className="flex justify-center items-center gap-2 text-xs pb-4 mt-0">
          {donationTypes.map((type) => (
            <label key={type} className="flex items-center gap-1 cursor-pointer">
              <input type="radio" name={`donationType-${amount || 'custom'}`} value={type} className="hidden peer"
